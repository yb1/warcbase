...
object ExtractClusters {
  ...
  def apply(records: RDD[ArchiveRecord], sc: SparkContext, k: Int = 20, maxIterations: Int = 120, numVocabs: Int = 1000,
    ...
    val topVocab = lemmatized.flatMap(x=>x).map(x=>(x,1)).reduceByKey(_+_).takeOrdered(numVocabs)(Ordering[Int].on(x => -x._2)).map(_._1).toSet

    val lemmas = lemmatized.map(x=>x.filter(word=>topVocab.contains(word))).persist()
    val contents = rec.map(r=>("http://web.archive.org/web/" + r.getCrawlDate + "/" + r.getUrl)).persist()
    val merged = lemmas.zip(contents).filter(x=>x._1.length > 0).persist()
    printf(s" # of docs = ${merged.count()}")
    val filtered_lemmas = merged.map(_._1).persist()
    val filtered_contents = merged.map(_._2).persist()

    val tfidf = getTfIdf(filtered_lemmas, minDocThreshold, numVocabs).cache()

    if (testK)
      runTestK(tfidf, maxIterations, maxK, sc, stepK, minK)

    new KMeansArchiveCluster(KMeans.train(tfidf, k, maxIterations), tfidf, filtered_lemmas, filtered_contents)
  }
